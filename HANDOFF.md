...
**验证方式的限制**：本地起静态服务器后，Claude 的浏览器面板读不到 `Documents` 下的文件（沙箱限制，表现为全部 404），因此**没有做成浏览器内的点击验证**。上述渲染验证是用 Node 把 `recommend.js` 当浏览器脚本执行、检查真实 HTML 输出完成的。下次不必再尝试 preview 服务器，直接用 Node 验证渲染 + 真机验证交互。
